...
st.set_page_config(page_title="HidroSed v3.0 Dios Supremo", page_icon="🌊", layout="wide")
...
st.sidebar.title("HidroSed v3.0 Supremo")
...
st.markdown(
    """
<div class='hs-hero'>
  <h1>🌊 HidroSed Maestra Integral v3.0 · Dios Supremo</h1>
...
with tabs[0]:
    ...
    if has("topo_support_df"):
        st.caption("Muestra de curvas topográficas de apoyo")
        st.dataframe(st.session_state["topo_support_df"].head(100), use_container_width=True)
...
"""
        ...
        use_topo = has("topo_support_df")
        if use_topo:
            st.success("Hay curvas topográficas de apoyo disponibles. Se usarán para mejorar cotas cercanas.")
        else:
            st.info("No hay curvas de apoyo cargadas. Las secciones se generarán solo con DEM.")
